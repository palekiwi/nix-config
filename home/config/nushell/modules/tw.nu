...
def get-pr-number [] {
    # Try GH_PR_NUMBER (in git dir, worktree-safe) first
    let git_dir = (^git rev-parse --git-dir 2>/dev/null | str trim)
    let pr_file = ($git_dir | path join GH_PR_NUMBER)
    if ($pr_file | path exists) {
        open $pr_file | str trim
    } else {
        # Fallback to gh CLI
        try {
